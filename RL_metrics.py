...
from sklearn.metrics import (auc, average_precision_score, confusion_matrix,
                             f1_score, precision_recall_curve, roc_curve,accuracy_score,recall_score,precision_score)
...
#try np.max in addition to argmax
def network_prediction(network, X: np.ndarray) -> dict:
    """Computes y_pred using a given network.
    Input is array of data entries.
    :param network: The network to use to calculate metrics
    :type  network: (Q)Network
    ...
    q, _ = network(X, step_type=constant([time_step.StepType.FIRST] * X.shape[0]), training=False)
    #print(q.numpy().shape)
    return q.numpy()  # Max action for each x in X
