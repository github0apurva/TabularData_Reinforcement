...
from RL_metrics import (classification_metrics, decision_function,network_prediction,
                            network_predictions, plot_pr_curve, plot_roc_curve,imbalance_ratio)
from RL_env import ClassifierEnv
...
class TrainDDQN():
    ...
    def fit(self, X_train, y_train, layers: list = [], imb_ratio: float = None, loss_fn=common.element_wise_squared_loss) -> None:
    ...
    def predict_proba(self, X_test):
        ...
        y_pred_try = network_prediction(network, X_test)
        return y_pred_try
    def save_network(self):
        handle = dataiku.Folder('models')
        """Saves Q-network as pickle to `model_path`."""
        with handle.get_writer(self.model_path) as f:  # Save Q-network as pickle
    ...
    def load_network(fp: str):
        ...
        """
        handle = dataiku.Folder('models')
        with handle.get_download_stream(fp) as f:  # Load the Q-network
